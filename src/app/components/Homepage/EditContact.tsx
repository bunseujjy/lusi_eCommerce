...
import { TUpdateContact, updateContact } from "@/types/type";
...
const EditContact: React.FC<contactProps> = ({ contact, user }) => {
  ...
  const {
    ...
    resolver: zodResolver(updateContact),
    defaultValues: {
  ...
  const handleEdit = async (data: TUpdateContact | any) => {
    setLoading(true);
    try {
      const ratingData = { ...data, userId: user.id };
      ...
      if (res.status === 200) {
        ...
        toast.error("You're not owner of this contact!");
      }
    } catch (error) {
    ...
    <div className="">
      <form onSubmit={handleSubmit(handleEdit)}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
          <div className="mx-auto overflow-hidden rounded-lg bg-white shadow-xl sm:w-full sm:max-w-xl dark:bg-navy-800">
                          ...
                          <div className="flex flex-wrap items-center mb-4 lg:mb-0">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-medium text-gray-400 dark:text-gray-400">
                                {moment(contact?.createdDate).format("LL")}
                              </p>
                            </div>
                          </div>
                          <div>
                            ...
                            <p className="text-xs font-thin text-gray-400 dark:text-gray-400">
                              {moment(contact?.createdDate).fromNow()}
                            </p>
                          </div>
                        </div>
